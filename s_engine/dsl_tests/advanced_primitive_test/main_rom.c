// main_rom.c — advanced_primitive_test via M-port const-ROM path
#define _GNU_SOURCE
#include <time.h>
#include <stdio.h>
#include <stdlib.h>
...
extern const s_engine_rom_t advanced_primitive_test_module_rom;
...
int main(void) {
    ...
    printf("=== advanced_primitive_test (M-port ROM) ===\n");
    s_expr_allocator_t alloc = { .malloc=bump_malloc, .free=bump_free, .ctx=NULL, .get_time=utc_realtime };
    s_expr_module_t module;
    if (s_engine_init_rom(&module, &advanced_primitive_test_module_rom, alloc) != S_EXPR_ERR_OK) {
