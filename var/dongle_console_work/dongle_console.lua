...
-- RS-485 passthrough (SERCOM4 9-bit MPCM, D6=TX/D7=RX). Always compiled into
-- the dongle build as diagnostics. Received frames arrive async as
-- OP_RS485_FRAME_RX (0x0014), not in a shell reply.
local CMD_RS485_CONFIG       = 0x0150
local CMD_RS485_SEND_FRAME   = 0x0151
local CMD_RS485_STATS        = 0x0152
-- Bus management (bus_controller only): Pi registers the RS-485 slave roster the
-- BC autonomously polls. Stage 2 = roster + command surface (no poll engine yet).
local CMD_BUS_REGISTER_SLAVE   = 0x0160
local CMD_BUS_UNREGISTER_SLAVE = 0x0161
local CMD_BUS_LIST_SLAVES      = 0x0162
local CMD_BUS_SET_POLL         = 0x0163
local CMD_BUS_POLL_ENABLE      = 0x0164
local CMD_BUS_CLEAR_ROSTER     = 0x0165
...
-- --target <addr>: route OP_SHELL_EXEC through a bus_controller to this RS-485
-- slave addr (the frame addr selects the slave). nil = talk to the local node.
local g_target_addr = nil
...
local function parse_args(argv)
    ...
    while i <= #argv do
        ...
        elseif a == "--target"   then
            i = i + 1
            local t = tonumber(argv[i])
            if not t or t < 1 or t > 254 then die("--target expects an RS-485 slave addr 1..254 (got %s)", tostring(argv[i])) end
            g_target_addr = t   -- route OP_SHELL_EXEC to this slave via the bus_controller
        ...
        elseif a == "--send-shell-bus-register" then
            -- Args: ADDR  CLASS_ID(hex/dec)  [FLAGS=0]
            -- Registers an RS-485 slave in the BC roster (Stage 2). flags bit0=TCP,
            -- bit1=enabled. Reply: reason:u8 (0=OK), roster_count:u8 on OK.
            i = i + 1; local addr  = tonumber(argv[i])
            i = i + 1; local cls   = tonumber(argv[i])
            local flags = 0
            if i + 1 <= #argv and not argv[i+1]:match("^%-%-") then
                i = i + 1; flags = tonumber(argv[i]) or 0
            end
            if not addr or addr < 1 or addr > 254 then die("bus-register ADDR 1..254") end
            if not cls then die("bus-register needs CLASS_ID (hex 0x.. or dec)") end
            local req_id = alloc_shell_req()
            local cmd_id = CMD_BUS_REGISTER_SLAVE
            local payload = {
                bit.band(req_id, 0xFF), bit.band(bit.rshift(req_id, 8), 0xFF),
                bit.band(cmd_id, 0xFF), bit.band(bit.rshift(cmd_id, 8), 0xFF),
                addr,
                bit.band(cls, 0xFF), bit.band(bit.rshift(cls, 8), 0xFF),
                bit.band(bit.rshift(cls, 16), 0xFF), bit.band(bit.rshift(cls, 24), 0xFF),
                bit.band(flags, 0xFF),
            }
            table.insert(opts.send_seq, {
                cmd = 0x0109,
                label = string.format("OP_SHELL_EXEC(bus_register addr=%d class=0x%08X flags=0x%02X)", addr, cls, flags),
                payload = payload, shell_req = req_id, shell_cmd = cmd_id,
            })
        elseif a == "--send-shell-bus-unregister" then
            -- Args: ADDR.  Reply: roster_count:u8 (CMD_FAILED if absent).
            i = i + 1; local addr = tonumber(argv[i])
            if not addr or addr < 1 or addr > 254 then die("bus-unregister ADDR 1..254") end
            local req_id = alloc_shell_req()
            local cmd_id = CMD_BUS_UNREGISTER_SLAVE
            local payload = {
                bit.band(req_id, 0xFF), bit.band(bit.rshift(req_id, 8), 0xFF),
                bit.band(cmd_id, 0xFF), bit.band(bit.rshift(cmd_id, 8), 0xFF),
                addr,
            ...
            table.insert(opts.send_seq, {
                cmd = 0x0109,
                label = string.format("OP_SHELL_EXEC(bus_unregister addr=%d)", addr),
                payload = payload, shell_req = req_id, shell_cmd = cmd_id,
            })
        elseif a == "--send-shell-bus-list" then
            -- Args: none.  Reply decoded by the OP_SHELL_REPLY handler (roster table).
            local req_id = alloc_shell_req()
            local cmd_id = CMD_BUS_LIST_SLAVES
            local payload = {
                bit.band(req_id, 0xFF), bit.band(bit.rshift(req_id, 8), 0xFF),
                bit.band(cmd_id, 0xFF), bit.band(bit.rshift(cmd_id, 8), 0xFF),
            }
            table.insert(opts.send_seq, {
                cmd = 0x0109, label = "OP_SHELL_EXEC(bus_list_slaves)",
                payload = payload, shell_req = req_id, shell_cmd = cmd_id,
            })
        elseif a == "--send-shell-bus-set-poll" then
            -- Args: PERIOD_MS  MAX_MISSES  TCP_RETRIES
            i = i + 1; local period  = tonumber(argv[i])
            i = i + 1; local misses  = tonumber(argv[i])
            i = i + 1; local retries = tonumber(argv[i])
            if not period or period < 1 or period > 65535 then die("bus-set-poll PERIOD_MS 1..65535") end
            if not misses or misses < 1 or misses > 255 then die("bus-set-poll MAX_MISSES 1..255") end
            if not retries or retries < 0 or retries > 255 then die("bus-set-poll TCP_RETRIES 0..255") end
            local req_id = alloc_shell_req()
            local cmd_id = CMD_BUS_SET_POLL
            local payload = {
                bit.band(req_id, 0xFF), bit.band(bit.rshift(req_id, 8), 0xFF),
                bit.band(cmd_id, 0xFF), bit.band(bit.rshift(cmd_id, 8), 0xFF),
                bit.band(period, 0xFF), bit.band(bit.rshift(period, 8), 0xFF),
                misses, retries,
            }
            table.insert(opts.send_seq, {
                cmd = 0x0109,
                label = string.format("OP_SHELL_EXEC(bus_set_poll %dms misses=%d retries=%d)", period, misses, retries),
                payload = payload, shell_req = req_id, shell_cmd = cmd_id,
            })
        elseif a == "--send-shell-bus-poll-enable" then
            -- Args: ENABLE (0|1)
            i = i + 1; local en = tonumber(argv[i])
            if not en or (en ~= 0 and en ~= 1) then die("bus-poll-enable ENABLE 0|1") end
            local req_id = alloc_shell_req()
            local cmd_id = CMD_BUS_POLL_ENABLE
            local payload = {
                bit.band(req_id, 0xFF), bit.band(bit.rshift(req_id, 8), 0xFF),
                bit.band(cmd_id, 0xFF), bit.band(bit.rshift(cmd_id, 8), 0xFF),
                en,
            ...
            table.insert(opts.send_seq, {
                cmd = 0x0109,
                label = string.format("OP_SHELL_EXEC(bus_poll_enable %d)", en),
                payload = payload, shell_req = req_id, shell_cmd = cmd_id,
            })
        elseif a == "--send-shell-bus-clear" then
            -- Args: none.  Empties the roster.
            local req_id = alloc_shell_req()
            local cmd_id = CMD_BUS_CLEAR_ROSTER
            local payload = {
                bit.band(req_id, 0xFF), bit.band(bit.rshift(req_id, 8), 0xFF),
                bit.band(cmd_id, 0xFF), bit.band(bit.rshift(cmd_id, 8), 0xFF),
            }
            table.insert(opts.send_seq, {
                cmd = 0x0109, label = "OP_SHELL_EXEC(bus_clear_roster)",
                payload = payload, shell_req = req_id, shell_cmd = cmd_id,
            })
        ...
        elseif a == "--send-shell-rs485-config" then
            -- Args: BAUD  MY_ADDR  [FLAGS]
            -- MY_ADDR=0xFF -> sniffer/listen-all; BAUD=0 -> leave unchanged.
            i = i + 1; local baud    = tonumber(argv[i])
            i = i + 1; local my_addr = tonumber(argv[i])
            local flags = 0
            if i + 1 <= #argv and not argv[i+1]:match("^%-%-") then
                i = i + 1; flags = tonumber(argv[i])
            end
            if not baud or baud < 0 or baud > 0xFFFFFFFF then die("rs485-config BAUD 0..2^32-1") end
            if not my_addr or my_addr < 0 or my_addr > 255 then die("rs485-config MY_ADDR 0..255") end
            if not flags or flags < 0 or flags > 255 then die("rs485-config FLAGS 0..255") end
            local req_id = alloc_shell_req()
            local cmd_id = CMD_RS485_CONFIG
            local payload = {
                bit.band(req_id, 0xFF), bit.band(bit.rshift(req_id, 8), 0xFF),
                bit.band(cmd_id, 0xFF), bit.band(bit.rshift(cmd_id, 8), 0xFF),
                bit.band(baud, 0xFF), bit.band(bit.rshift(baud, 8), 0xFF),
                bit.band(bit.rshift(baud, 16), 0xFF), bit.band(bit.rshift(baud, 24), 0xFF),
                my_addr, flags,
            }
            table.insert(opts.send_seq, {
                cmd = 0x0109,
                label = string.format("OP_SHELL_EXEC(rs485_config baud=%d my_addr=0x%02X flags=0x%02X)",
                                      baud, my_addr, flags),
                payload = payload, shell_req = req_id, shell_cmd = cmd_id,
            })
        elseif a == "--send-shell-rs485-stats" then
            -- No args. Reply: rx_words:u32, frames_ok:u32, crc_fail:u32, overrun:u32.
            local req_id = alloc_shell_req()
            local cmd_id = CMD_RS485_STATS
            table.insert(opts.send_seq, {
                cmd = 0x0109,
                label = "OP_SHELL_EXEC(rs485_stats)",
                payload = {
                    bit.band(req_id, 0xFF), bit.band(bit.rshift(req_id, 8), 0xFF),
                    bit.band(cmd_id, 0xFF), bit.band(bit.rshift(cmd_id, 8), 0xFF),
                },
        ...
        elseif a == "--send-shell-rs485-send-frame" then
            -- Args: DEST SRC TYPE SEQ  [DATA_BYTE ...]   (all accept hex/decimal)
            -- Raw structured-frame injector; the chip appends the CRC-8.
            -- TYPE: 0=NO_MSG 1=POLL 2=DATA 3=ACK 4=NAK (+0x10 = TCP flag).
            local hdr = {}
            for _, name in ipairs({"DEST", "SRC", "TYPE", "SEQ"}) do
                i = i + 1; local v = tonumber(argv[i])
                if not v or v < 0 or v > 255 then die("rs485-send-frame %s 0..255 (got %s)", name, tostring(argv[i])) end
                table.insert(hdr, v)
            end
            local data = {}
            while i + 1 <= #argv and not argv[i+1]:match("^%-%-") do
                i = i + 1
                local v = tonumber(argv[i])
                if not v or v < 0 or v > 255 then die("rs485-send-frame DATA byte 0..255 (got %s)", argv[i]) end
                table.insert(data, v)
            end
            if #data > 120 then die("rs485-send-frame payload <= 120 bytes") end
            local req_id = alloc_shell_req()
            local cmd_id = CMD_RS485_SEND_FRAME
            local payload = {
                bit.band(req_id, 0xFF), bit.band(bit.rshift(req_id, 8), 0xFF),
                bit.band(cmd_id, 0xFF), bit.band(bit.rshift(cmd_id, 8), 0xFF),
                hdr[1], hdr[2], hdr[3], hdr[4],     -- dest, src, type, seq
            }
            for _, b in ipairs(data) do table.insert(payload, b) end
            table.insert(opts.send_seq, {
                cmd = 0x0109,
                label = string.format("OP_SHELL_EXEC(rs485_send_frame dest=0x%02X src=0x%02X type=0x%02X seq=%d, %d bytes)",
                    hdr[1], hdr[2], hdr[3], hdr[4], #data),
                payload = payload, shell_req = req_id, shell_cmd = cmd_id,
            })
...
local OPCODE_NAMES = {
    ...
    [0x0013] = "OP_EVENT",
    [0x0014] = "OP_RS485_FRAME_RX",
    [0x0015] = "OP_BUS_SLAVE_DOWN",
    [0x0016] = "OP_BUS_SLAVE_UP",
    -- m2s (host -> dongle)
...
local function encode_m2s(cmd, payload, addr)
    payload = payload or {}
    addr = addr or 0x00
    local seq  = g_tx_seq
...
local function send_m2s_frame(fd, entry)
    -- --target routes OP_SHELL_EXEC (0x0109) to an RS-485 slave via the
    -- bus_controller bridge: the frame addr selects the slave. Local/ladder
    -- frames (and everything when --target is unset) stay addr=0x00.
    local addr = 0x00
    if g_target_addr and entry.cmd == 0x0109 then addr = g_target_addr end
    local wire, seq = encode_m2s(entry.cmd, entry.payload, addr)
    ...
    if len > 0 then
        ...
        elseif cmd == 0x0011 and len >= 3 then
            ...
            elseif status == 0 and expected_cmd == CMD_DAC_STOP and len == 3 then
                io.write("\n  dac_stop: ok")
                pending_shell_requests[req_id] = nil
            elseif status == 0 and expected_cmd == CMD_RS485_STATS and len == 3 + 16 then
                local function u32(o) return bit.bor(f[o], bit.lshift(f[o+1],8),
                                                     bit.lshift(f[o+2],16), bit.lshift(f[o+3],24)) end
                io.write(string.format("\n  rs485_stats: rx_words=%d frames_ok=%d crc_fail=%d overrun=%d",
                    u32(11), u32(15), u32(19), u32(23)))
                pending_shell_requests[req_id] = nil
            elseif status == 0 and expected_cmd == CMD_BUS_REGISTER_SLAVE and len == 3 + 2 then
                io.write(string.format("\n  bus_register: OK  roster_count=%d", f[12]))
                pending_shell_requests[req_id] = nil
            elseif expected_cmd == CMD_BUS_REGISTER_SLAVE and status == 3 and len == 3 + 1 then
                local REASON = {[1]="ROSTER_FULL", [2]="DUPLICATE_ADDR", [3]="BAD_ADDR"}
                io.write(string.format("\n  bus_register: FAILED (%s)", REASON[f[11]] or ("reason="..f[11])))
                pending_shell_requests[req_id] = nil
            elseif status == 0 and expected_cmd == CMD_BUS_UNREGISTER_SLAVE and len == 3 + 1 then
                io.write(string.format("\n  bus_unregister: OK  roster_count=%d", f[11]))
                pending_shell_requests[req_id] = nil
            elseif status == 0 and expected_cmd == CMD_BUS_LIST_SLAVES and len >= 3 + 2 then
                -- reply: total:u8, shown:u8, then `shown` x 10-byte rows.
                ...
                local function u32at(o) return bit.bor(f[o], bit.lshift(f[o+1], 8),
                                              bit.lshift(f[o+2], 16), bit.lshift(f[o+3], 24)) end
                local total, shown = f[11], f[12]
                local STATE = {[0]="UNKNOWN", [1]="ALIVE", [2]="DEAD"}
                io.write(string.format("\n  bus_list: %d slave(s)%s",
                    total, (shown < total) and string.format(" (showing %d)", shown) or ""))
                for r = 0, shown - 1 do
                    local o = 13 + r * 10
                    local addr, cls = f[o], u32at(o + 1)
                    local flags, st, miss = f[o + 5], f[o + 6], f[o + 7]
                    local ago = u16at(o + 8)
                    io.write(string.format(
                        "\n    addr=%-3d class=0x%08X flags=0x%02X[%s%s] state=%-7s misses=%d last_seen=%s",
                        addr, cls, flags,
                        (bit.band(flags,1) ~= 0) and "TCP" or "UDP",
                        (bit.band(flags,2) ~= 0) and ",EN" or "",
                        STATE[st] or "?", miss,
                        (ago == 0xFFFF) and "never" or (ago .. "ms ago")))
                end
                pending_shell_requests[req_id] = nil
            elseif status == 0 and (expected_cmd == CMD_BUS_SET_POLL
                                 or expected_cmd == CMD_BUS_POLL_ENABLE
                                 or expected_cmd == CMD_BUS_CLEAR_ROSTER) and len == 3 then
                io.write("\n  bus: ok")
                pending_shell_requests[req_id] = nil
        ...
        elseif cmd == 0x0014 and len >= 3 then
            -- OP_RS485_FRAME_RX (sniffer): { from_addr:u8, type:u8, seq:u8, payload bytes }
            local from_addr, ftype, fseq = f[8], f[9], f[10]
            local ftname = ({[0]="NO_MSG",[1]="POLL",[2]="DATA",[3]="ACK",[4]="NAK"})[ftype % 16] or "?"
            local tcp = (ftype >= 16) and "+TCP" or ""
            local phex, pasc = {}, {}
            for i = 11, 7 + len do
                table.insert(phex, string.format("%02x", f[i]))
                table.insert(pasc, ascii_safe(f[i]))
            end
            io.write(string.format("\n  rs485 rx: from=0x%02X  type=%s%s(0x%02X) seq=%d  len=%d  payload: %s  |%s|",
                from_addr, ftname, tcp, ftype, fseq, len - 3, table.concat(phex, " "), table.concat(pasc, "")))
        elseif cmd == 0x0015 and len >= 1 then
            -- OP_BUS_SLAVE_DOWN: { addr:u8 } — BC poll engine lost a slave.
            io.write(string.format("\n  *** BUS SLAVE DOWN: addr=%d ***", f[8]))
        elseif cmd == 0x0016 and len >= 5 then
            -- OP_BUS_SLAVE_UP: { addr:u8, class_id:u32 } — slave recovered.
            local cls = bit.bor(f[9], bit.lshift(f[10], 8), bit.lshift(f[11], 16), bit.lshift(f[12], 24))
            io.write(string.format("\n  *** BUS SLAVE UP: addr=%d class=0x%s ***", f[8], bit.tohex(cls):upper()))
