// ============================================================================
// user_functions.c — register_dongle (Seeed XIAO RA4M1 — step 3b port)
//
// Real implementations of the chain's user-defined functions. Per-chip copy
// of the SAMD21 reference: all chain handlers (register, heartbeat, manifest,
// NAK, the three commissioning handlers, shell dispatch, internal events) are
// chip-agnostic and ported verbatim. Three spots are RA4M1-specific:
//   * ra4m1_read_uid()  — FSP unique-ID read (was samd21_read_uid)
//   * toggle_led()      — RA4M1 liveness counter (LED pin not yet verified)
//   * REGISTER_PID      — RA4M1 register_dongle USB PID
//
...
#include "bsp_api.h"          // FSP BSP: R_BSP_UniqueIdGet()
...
// ----------------------------------------------------------------------------
// RA4M1 chip UID. The R7FA4M1AB carries a 128-bit factory unique ID; FSP
// exposes it via R_BSP_UniqueIdGet(), which returns a 16-byte little-endian
// id. Cleaner than the SAMD21 port's fixed-address reads.
//
// TODO-verify (Pi): the accessor + struct field name against the vendored
// FSP headers — R_BSP_UniqueIdGet() returning bsp_unique_id_t with a
// .unique_id_bytes[16] member. See register_dongle/README.md.
// ----------------------------------------------------------------------------
static void ra4m1_read_uid(uint8_t out[16]) {
    bsp_unique_id_t const* id = R_BSP_UniqueIdGet();
    for (int i = 0; i < 16; i++) {
        out[i] = id->unique_id_bytes[i];
    }
}
...
//   [10..25]  chip_uid             RA4M1 16-byte factory unique ID
//   [26..27]  vid                  0x2886
//   [28..29]  pid                  0x0053
...
#define REGISTER_PID             0x0053U      // RA4M1 register_dongle PID
...
//
// Wire payload (from main.c via the shell_pending queue):
...
// (capacity >= COMM_PAYLOAD_MAX). Returns the reply length. Reused by both
// transports: the USB OP_SHELL_EXEC handler below and the RS-485 slave tunnel in
// main.c. Callers guarantee exec_len >= SHELL_EXEC_HEADER_LEN.
...
//
// On the SAMD21 this toggled the user LED (PA17). The XIAO RA4M1's user-LED
// pin is not yet verified against the copied uno_r4 board definition — the
// same open question blink_frame deferred (see the RA4M1 bring-up memory and
// ra4m1_pin_map). Until it is, this is a liveness counter rather than a GPIO
// toggle: g_engine_calls climbs once per tick, observable via the debugger.
// Wiring the real activity LED is a step-4 item alongside the chip pin map.
//
// Must return SE_PIPELINE_CONTINUE so the fork keeps the branch alive across
// ticks.
// ----------------------------------------------------------------------------
