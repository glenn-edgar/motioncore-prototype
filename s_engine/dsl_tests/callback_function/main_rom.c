// main_rom.c — callback_function via M-port const-ROM path
#define _GNU_SOURCE
#include <time.h>
#include <stdio.h>
#include <stdlib.h>
...
extern const s_engine_rom_t callback_function_module_rom;
...
int main(void) {
    printf("=== callback_function (M-port ROM) ===\n");
    s_expr_allocator_t alloc = { .malloc=bump_malloc, .free=bump_free, .ctx=NULL, .get_time=utc_realtime };
    s_expr_module_t module;
    if (s_engine_init_rom(&module, &callback_function_module_rom, alloc) != S_EXPR_ERR_OK) {
    ...
    s_expr_tree_instance_t* tree = s_expr_tree_create_by_hash(&module, CALLBACK_FUNCTION_HASH, 0);
