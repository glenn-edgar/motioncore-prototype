...
exec docker run -d \
    ...
    -e DASHBOARD_URL="${DASHBOARD_URL:-http://192.168.1.66:28081/irrigation/alerts}" \
    ...
    -e KB3_FLOW_ARM="${KB3_FLOW_ARM:-0}" \
    -e KB4_CURVE_ARM="${KB4_CURVE_ARM:-0}" \
    -e KB1_EQ_KILL_A="${KB1_EQ_KILL_A:-1.8}" \
    -e KB3_FOUL_ARM="${KB3_FOUL_ARM:-0}" \
    -e KB3_PLC_LOW_GPM="${KB3_PLC_LOW_GPM:-3.0}" \
    -e KB3_HUN_FLOW_GPM="${KB3_HUN_FLOW_GPM:-4.0}" \
    -e KB3_FILTER_COOLDOWN_STEPS="${KB3_FILTER_COOLDOWN_STEPS:-1}" \
    -e KB3_FLOW_ABS_FLOOR_GPM="${KB3_FLOW_ABS_FLOOR_GPM:-5.0}" \
    -e KB3_FLOWSTEP_ARM="${KB3_FLOWSTEP_ARM:-1}" \
    -e KB3_FLOWSTEP_GPM="${KB3_FLOWSTEP_GPM:-1.5}" \
    -e KB2_STEP_ARM="${KB2_STEP_ARM:-0}" \
    -e KB2_COHORT_ARM="${KB2_COHORT_ARM:-0}" \
    "$IMAGE_TAG"
