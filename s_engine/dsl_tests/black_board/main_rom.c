// main_rom.c — black_board via M-port const-ROM path
// Per-tree binary: set BB_TREE_HASH at compile time to choose which tree to run.
#define _GNU_SOURCE
#include <time.h>
#include <stdio.h>
#include <stdlib.h>
...
#ifndef BB_TREE_HASH
#define BB_TREE_HASH DEMO_BLACKBOARD_ACCESS_HASH
#endif
#ifndef BB_TREE_NAME
#define BB_TREE_NAME "demo_blackboard_access"
#endif

extern const s_engine_rom_t black_board_module_rom;
...
static void* bump_malloc(void* ctx, size_t size) {
    ...
    if (g_bump_used + aligned > BUMP_BUFFER_SIZE) { fprintf(stderr,"bump OOM (need %zu, have %zu)\n", aligned, BUMP_BUFFER_SIZE-g_bump_used); return NULL; }
...
int main(void) {
    ...
    s_expr_tree_instance_t* tree = s_expr_tree_create_by_hash(&module, BB_TREE_HASH, 0);
    if (!tree) { printf("FATAL: create tree %s\n", BB_TREE_NAME); return 1; }
