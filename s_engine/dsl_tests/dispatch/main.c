...
#include "dispatch_test_bin_32.h"
#include "dispatch_test_records.h"

extern void dispatch_test_register_all(s_expr_module_t* module);
...
static void register_user_functions(s_engine_handle_t* engine) {
    dispatch_test_register_all(&engine->module);
}
...
int main(int argc, char* argv[]) {
    ...
    bool result = s_engine_load_from_rom(
        &engine,
        &alloc,
        dispatch_test_module_bin_32,
        DISPATCH_TEST_MODULE_BIN_32_SIZE,
        debug_callback,
        1,
        user_fns
    );
