// ============================================================================
// blink_frame — Seeed XIAO RA4M1 (Renesas R7FA4M1AB, Cortex-M4)
//
// Same s_engine M-port chain as blink_engine, but the 1 Hz host-visible
// heartbeat is a libcomm s2m frame (SLIP + CRC-8/AUTOSAR) instead of a text
// line. Proves the vendored libcomm framing compiles and round-trips on
// Cortex-M4 — the libcomm-portability milestone before register_dongle.
// ============================================================================
...
// board_millis() ms -> seconds. Divisor is (double)1000, not 1000.0 — the RA
// build uses -fsingle-precision-constant (see blink_engine/Makefile note).
...
// 256 B is power-of-2 (frame_ring_init requires it) and fits one ~30 B
// encoded frame with margin.
// ----------------------------------------------------------------------------
...
int main(void) {
    ...
    for (;;) {
        ...
        if ((int32_t)(now - next_frame_ms) >= 0) {
            next_frame_ms += 1000;
            static const uint8_t payload[] = "hello ra4m1 frame";
            frame_meta_t meta = {
                .addr        = 1,
                .cmd         = 0x0001,
        ...
        // Drain the ring to CDC. CFG_TUD_CDC_TX_BUFSIZE is 64 B; a 64 B local
        // buffer keeps each drained chunk within one tud_cdc_write call.
