...
// stop progressing, chip resets in ~1.09 s and recovers itself.
//
// IWDT clock is IWDTLOCO (15 kHz, independent of main clock — survives clock
// failures). Timeout: TOPS=01 (512 cycles) × CKS=1111 (/128 div) / 15 kHz =
// 1.09 s. 4× margin over 250 ms pet cadence; bites if a chain step blocks
// for >1 s. IWDTCR writes only take effect in register-start mode (OFS0[1]=1);
// the erased-flash default is register-start, but XIAO bootloader may differ.
...
// intentionally truncated — slice-1 only needs the bottom three RSTSR1 flags
// (IWDTRF/WDTRF/SWRF). Bit map of the returned uint16:
//   bit 0 = PORF     (power-on)
//   bit 1 = LVD0RF   bit 2 = LVD1RF    bit 3 = LVD2RF
//   bit 8 = IWDTRF   (** the goertzel-hang signal **)
//   bit 9 = WDTRF    (regular WDT — unused in this build)
//   bit 10 = SWRF    (software reset: NVIC_SystemReset / DFU touch)
// Host decodes via OP_DBG_LOG output on first post-boot frame.

void     hal_capture_reset_cause(void);   // call ONCE, very early in main()
uint16_t hal_get_reset_cause(void);       // returns the captured snapshot
